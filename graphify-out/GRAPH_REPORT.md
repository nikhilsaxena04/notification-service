...
## Corpus Check
- 36 files · ~18,253 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 502 nodes · 549 edges · 37 communities (21 shown, 16 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 29 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4ef015a5`
...
- `main()` --calls--> `Load()`  [INFERRED]
  cmd/router/main.go → pkg/config/config.go
- `main()` --calls--> `Load()`  [INFERRED]
...
Cohesion: 0.05
Nodes (13): BatchSendRequest, BatchSendResponse, DispatchRequest, DispatchRequest_Email, DispatchRequest_Push, DispatchRequest_Sms, isDispatchRequest_Payload, isSendNotificationRequest_Payload (+5 more)

### Community 1 - "Community 1"
Cohesion: 0.07
Nodes (5): Channel, file_notification_proto_rawDescGZIP(), Priority, SMSPayload, Status

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (29): 10. Idempotency via Redis `SET NX`, 11. UUIDv7 for Server-Assigned Job IDs, 12. Strict Layer Isolation (Hexagonal DI in Practice), 13. Blocking Pop with Periodic Unblocking, 14. Dual-Context Graceful Shutdown, 15. Exponential Backoff with Jitter, 16. Deterministic JSON for `LREM` (Exact-Match Removal), 17. `sync.WaitGroup` as a Goroutine Lifecycle Fence (+21 more)

### Community 3 - "Community 3"
Cohesion: 0.07
Nodes (4): Channel, DispatchResponse, Priority, Status
...
Cohesion: 0.08
Nodes (24): 1. Start local infrastructure, 2. Run the services (three terminals), 3. Send a notification, Architecture, code:block1 (Client), code:block2 (notification-service/), code:bash (docker compose up -d), code:bash (# Terminal 1 — notification-router (must start first, worker) (+16 more)

### Community 9 - "Community 9"
Cohesion: 0.10
Nodes (12): main(), Service, NewService(), Config, Load(), Producer, NewProducer(), setupTestRedis() (+4 more)
...
Cohesion: 0.17
Nodes (9): main(), MockEmailProvider, MockPushProvider, MockSMSProvider, Provider, NewMockEmailProvider(), NewMockPushProvider(), NewMockSMSProvider() (+1 more)

### Community 16 - "Community 16"
Cohesion: 0.20
Nodes (9): Added, Changelog, Core Job Struct (`pkg/queue/job.go`), Dispatch & DLQ — `notification-router` (Phase 4), gRPC Contracts (`proto/notification.proto`), Ingress Layer — `api-service` (Phase 2), Local Development Infrastructure (`docker-compose.yml`), [Unreleased] (+1 more)
...
## Knowledge Gaps
- **87 isolated node(s):** `command`, `args`, `command`, `args`, `isSendNotificationRequest_Payload` (+82 more)
...
- **Why does `SendNotificationRequest` connect `Community 13` to `Community 0`, `Community 1`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `SendNotificationRequest` connect `Community 14` to `Community 10`, `Community 29`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `DispatchRequest` connect `Community 0` to `Community 1`?**
